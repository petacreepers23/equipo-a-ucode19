...
def sincronizar_datos(fich):
    ...
    line_count, left_acc_x, left_acc_y, left_acc_z,frame = parser(fich)
    ...
    path_wav=input("Intrudzca la direccion del video. (fichero wav): ")
    tiempo_primera,tiempo_segunda,tiempo_tercera,tiempo_cuarta=detect_clap.procesar(path_wav)
    ...
    print(best_candidate_begin)
    print(best_candidate_end)
    print(video_synch_init)
    print(video_synch_end)
    sampling_rate=float(best_candidate_end-best_candidate_begin)/float(video_synch_end-video_synch_init)
    print(sampling_rate)
